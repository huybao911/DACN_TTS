...
import { MenuItem as MenuItemCity } from "@material-ui/core"
...
const Content: React.FC = (): JSX.Element => {
    ...
    const findIndexEvent = user.jobs.map((job: any) => job.nameJob)
    React.useEffect(() => {
        setJobDetails(() =>
            user?.jobs?.filter((job: any) =>
                job.nameJob == findIndexEvent[0]
            ));
    }, [user]);
    React.useEffect(() => {
                    ...
                    {jobs && jobs.length > 0 ? (
                        <Box style={{ display: "flex", flexDirection: "row" }}>
                            <Box style={{ paddingLeft: '20px', top: 100, zIndex: 10, }}>
                                {jobs.map((job: any) =>
                                    <Box>
                                        <Link style={{ textDecoration: "none", color:"black" }} to={`/event1/${job._id}`}>
