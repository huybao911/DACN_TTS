...
import { loginCompany } from "redux/actions/company";
...
const LoginUser: React.FC = (): JSX.Element => {
  ...
  const onHandleSubmit = (values: IInitialValues, { setSubmitting }: any) => {
    ...
    dispatch(loginCompany(values, setSubmitting));
  };
          ...
          <Box>
            <Box display={"flex"} flexDirection={'column'} justifyContent={'center'} style={{ backgroundColor: 'white', padding: '220px 0px 0px', borderRadius: '20px' }}>
              <Box display={"flex"} flexDirection={'column'} justifyContent={'center'} textAlign={'left'}>
                ...
                <Typography style={{ fontWeight: "500", fontSize: "20px", marginTop: "14px", marginBottom: "14px", letterSpacing: '0.6px', display:"flex", margin:"auto" }} >
                  Đăng nhập
                </Typography>
              </Box>
              <Box display={'flex'} textAlign={'left'} flexDirection={'row'} style={{ fontSize: "14px", fontWeight: '380', marginTop:"10px" }} >
                <Box style={{ color: 'black', marginRight: '4px' }}>
                  Bạn là người ứng tuyển ?
                </Box>
                <Box component={Link} to="/register"
                  style={{ color: 'rgb(84 219 154)', cursor: 'pointer', textDecoration: "none" }}
                >
                  Tạo tài khoản
                </Box>
              </Box>
              <Box display={'flex'} textAlign={'left'} flexDirection={'row'} style={{ fontSize: "14px", fontWeight: '380', marginTop: "10px" }} >
                ...
                <Box component={Link} to="/registerCompany"
                  style={{ color: '#3b78dc', cursor: 'pointer', textDecoration: "none" }}
                >
                  Tạo tài khoản
                </Box>
              </Box>
